...
const LIMIT = 10;
const API = '/internal/api/tugasrutin-laporan';
...
function TrDetailModal({ id, onClose }) {
  ...
  const doneCount = taskEntries.filter(([,t]) => t.done).length;
  const skipCount = taskEntries.filter(([,t]) => t.skip).length;
  const fotoCount = taskEntries.reduce((acc,[,t]) => acc + (t.photos?.length||0), 0);

  return (
    <>
      <div onClick={onClose} style={{position:'fixed',inset:0,background:'rgba(0,0,0,.72)',zIndex:1000}}/>
      <div style={{
        ...
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
          <div>
            ...
            <div style={{fontSize:11,color:'#8b8d93',marginTop:2,fontFamily:'monospace',textTransform:'uppercase',letterSpacing:1}}>
              {data?.user_name} · {fmtWib(data?.created_at)}
            </div>
          </div>
        ...
        {taskEntries.map(([kode, task]) => (
          <div key={kode} style={{
            ...
            borderLeft:`3px solid ${task.skip ? '#f5a623' : task.done ? '#4caf7d' : '#e5674f'}`,
...
export default function TugasRutinTab() {
  ...
  useEffect(() => {
    fetch(`${API}?outlets_only=1`)
      .then(r=>r.json()).then(d=>{ if(Array.isArray(d)) setOutletList(d); }).catch(()=>{});
  }, []);
  ...
  return (
    <div className={s.dcWrap}>
      <div className={s.dcHeadBar}>
        <div className={s.dcHeadLeft}>
          ...
          <div className={s.searchWrap}>
            ...
            <input type="text" placeholder="Search..." value={search} className={s.searchInput} onChange={e=>setSearch(e.target.value)}/>
      ...
      <div className={`${s.ledger} ${s.dcLedger}`}>
        <div className={`${s.lgRow} ${s.lgHead} ${s.trLgRow} ${s.h}`}>
          {COLS.map(c => {
            ...
            return (
              <span key={c.key} className={s.sortCol} onClick={() => toggleSort(c.key)}
        ...
        {!loading && sorted.map((row, idx) => (
          <div key={row.id} className={`${s.lgRow} ${s.trLgRow}`}>
            ...
            <span className={`${s.dcCell} ${row.done_count > 0 ? s.dcNormal : s.dcZero}`}>{row.done_count}</span>
            <span className={`${s.dcCell} ${row.skip_count > 0 ? s.dcTdkDikerjakan : s.dcZero}`}>{row.skip_count}</span>
      ...
      {detailId && <TrDetailModal id={detailId} onClose={() => setDetailId(null)} />}
    </div>
  );
}
