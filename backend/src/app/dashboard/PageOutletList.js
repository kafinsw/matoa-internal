...
function OutletForm({ initial, onSubmit, submitting, err }) {
  const [form, setForm] = useState(initial);
  function set(k, v) { setForm(f => ({ ...f, [k]: v })); }
  return (
    <form className={s.catModalForm} onSubmit={e => { e.preventDefault(); onSubmit(form); }}>
      <label className={s.catFormLabel}>Nama Outlet</label>
      <input className={s.catFormInput} value={form.nama} onChange={e => set('nama', e.target.value)} placeholder="Nama outlet..." />
      <label className={s.catFormLabel}>Status</label>
      <select className={s.catFormSelect} value={form.status} onChange={e => set('status', e.target.value)}>
...
function EditOutletForm({ initial, onSubmit, submitting, err }) {
  const [form, setForm] = useState(initial);
  function set(k, v) { setForm(f => ({ ...f, [k]: v })); }
  return (
    <form className={s.catModalForm} onSubmit={e => { e.preventDefault(); onSubmit(form); }}>
      <label className={s.catFormLabel}>Nama Outlet</label>
      <input className={s.catFormInput} value={form.nama} onChange={e => set('nama', e.target.value)} />
      <label className={s.catFormLabel}>Status</label>
      <select className={s.catFormSelect} value={form.status} onChange={e => set('status', e.target.value)}>
        <option value="active">Active</option>
        <option value="inactive">Inactive</option>
      ...
      {err && <span className={s.catFormErr}>{err}</span>}
      <button className={s.catFormSubmit} type="submit" disabled={submitting}>
        {submitting ? 'Menyimpan…' : 'OK'}
      </button>
    </form>
  );
}

function AddOutletModal({ onClose, onSaved }) {
  const [submitting, setSubmitting] = useState(false);
  const [err, setErr] = useState('');
  async function handleSubmit(form) {
    if (!form.nama) { setErr('Nama wajib diisi'); return; }
    setSubmitting(true); setErr('');
    try {
      const res = await fetch('/internal/api/outlets-crud', {
    ...
    <div className={s.catModalOverlay}>
      <div className={s.catModal}>
        <div className={s.catModalHead}>
          <span className={s.catModalTitle}>+ OUTLET</span>
          <button className={s.catModalClose} type="button" onClick={onClose}>✕</button>
        </div>
        <OutletForm initial={{ nama: '', status: 'active' }} onSubmit={handleSubmit} submitting={submitting} err={err} />
      </div>
    </div>
  );
}

function EditOutletModal({ row, onClose, onSaved }) {
  const [submitting, setSubmitting] = useState(false);
  const [err, setErr] = useState('');
  async function handleSubmit(form) {
    if (!form.nama) { setErr('Nama wajib diisi'); return; }
    ...
    try {
      const res = await fetch('/internal/api/outlets-crud', {
        ...
        body: JSON.stringify({ id: row.id, nama: form.nama, status: form.status }),
      });
      const data = await res.json();
      if (!res.ok) { setErr(`${data.error || 'Gagal simpan'} (HTTP ${res.status})`); return; }
      onSaved(); onClose();
  ...
  return (
    <div className={s.catModalOverlay}>
      <div className={s.catModal}>
        <div className={s.catModalHead}>
          <span className={s.catModalTitle}>✎ EDIT OUTLET</span>
          <button className={s.catModalClose} type="button" onClick={onClose}>✕</button>
        </div>
        <EditOutletForm initial={{ nama: row.nama ?? '', status: row.status ?? 'active' }} onSubmit={handleSubmit} submitting={submitting} err={err} />
      </div>
    </div>
  );
}

function DeleteOutletModal({ row, onClose, onConfirm }) {
  return (
    <div className={s.catModalOverlay}>
      <div className={`${s.catModal} ${s.catModalSm}`}>
        <div className={s.catDelTitle}>Ingin menghapus outlet ini?</div>
...
export default function PageOutletList() {
  ...
  const fetchData = useCallback(async (silent = false) => {
    try {
      const res = await fetch('/internal/api/outlets-crud', { cache: 'no-store' });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const data = await res.json();
      if (!Array.isArray(data)) throw new Error('bad response');
      const hash = data.map(r => r.id + ':' + r.nama + ':' + r.status).join('|');
  ...
  async function handleDeleteConfirm() {
    if (!deleteRow) return;
    try {
      const res = await fetch(`/internal/api/outlets-crud?id=${deleteRow.id}`, { method: 'DELETE' });
      const data = await res.json();
      if (!res.ok) { alert(`Gagal hapus: ${data.error || ''} (HTTP ${res.status})`); return; }
      lastHash.current = ''; fetchData();
      setToast('Outlet Berhasil Dihapus');
  ...
  const filtered   = rows.filter(r => !q || [r.nama, r.status].some(v => v?.toLowerCase().includes(q.toLowerCase())));
    ...
    <div className={`${s.wrap} ${s.outletWrap}`}>
      ...
      <div className={s.ledgerHead}>
        <div className={s.catHeadBar}>
          <div className={s.catHeadLeft}>
            <button className={s.catAddBtn} onClick={() => setShowAdd(true)}>+ OUTLET</button>
      ...
      <div className={`${s.ledger} ${s.outletLedger}`}>
        <div className={`${s.lgRow} ${s.h} ${s.outletLgRow}`}>
          ...
          <SortHdr label="STATUS" col="status" sortCol={sortCol} sortDir={sortDir} onSort={onSort} />
        ...
        {!loading && !error && pageRows.map((row, i) => (
          <div key={row.id} className={`${s.lgRow} ${s.outletLgRow}`}>
            <span className={s.catNo}>{(page - 1) * LIMIT + i + 1}</span>
            <span className={s.catGejala}>{row.nama || '—'}</span>
            <span className={s.catType}>
              <span className={`${s.typ} ${row.status === 'active' ? s.typActive : s.typInactive}`}>{row.status || '—'}</span>
            </span>
      ...
      {showAdd    && <AddOutletModal    onClose={() => setShowAdd(false)}    onSaved={() => { lastHash.current = ''; fetchData(); setToast('Outlet Berhasil Ditambahkan'); }} />}
      {editRow    && <EditOutletModal   row={editRow}   onClose={() => setEditRow(null)}    onSaved={() => { lastHash.current = ''; fetchData(); setToast('Outlet Berhasil Diubah'); }} />}
      {deleteRow  && <DeleteOutletModal row={deleteRow} onClose={() => setDeleteRow(null)}  onConfirm={handleDeleteConfirm} />}
