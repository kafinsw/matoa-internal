    ...
    switch ($route) {
        ...
        case '/tugasrutin-jadwal-harian':
            ...
            $rows = $pdo->query("
                ...
                WHERE tj.day_of_week = $dow AND tj.user_id = $uid AND tt.user_id = $uid" . ($outletFilter ? " AND tj.outlet_id = $outletFilter" : "") . "
        ...
        case '/daily/store':
            ...
            $created_at = $date_store ? ($date_store . ' ' . (new DateTime('now', new DateTimeZone('Asia/Jakarta')))->format('H:i:s')) : (new DateTime('now', new DateTimeZone('Asia/Jakarta')))->format('Y-m-d H:i:s');
